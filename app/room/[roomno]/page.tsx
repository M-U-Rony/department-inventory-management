...
import { Printer, Almari, Bookshelf, BaseRoomItem } from "../../../types/roomItems";
...
export default function Room() {
  ...
  const [selectedRoomItem, setSelectedRoomItem] = useState<null | { type: "printer" | "almari" | "bookshelf"; item: Printer | Almari | Bookshelf | null }>(null);
  ...
  const [assigningRoomItemId, setAssigningRoomItemId] = useState<number | null>(null);
      ...
      try {
        ...
        if (selectedRoomItem && selectedRoomItem.item) {
          if (selectedRoomItem.type === "printer") {
            const refreshed = (((data.room.printers as Printer[]) ?? [])).find((x: Printer) => x.id === selectedRoomItem.item!.id);
            if (refreshed) setSelectedRoomItem({ type: "printer", item: refreshed });
          } else if (selectedRoomItem.type === "almari") {
            const refreshed = (((data.room.almari as Almari[]) ?? [])).find((x: Almari) => x.id === selectedRoomItem.item!.id);
            if (refreshed) setSelectedRoomItem({ type: "almari", item: refreshed });
          } else if (selectedRoomItem.type === "bookshelf") {
            const refreshed = (((data.room.bookshelf as Bookshelf[]) ?? [])).find((x: Bookshelf) => x.id === selectedRoomItem.item!.id);
            if (refreshed) setSelectedRoomItem({ type: "bookshelf", item: refreshed });
          }
        }
  ...
  return (
    <main className="w-full max-w-6xl mx-auto p-4 space-y-6">
                  ...
                  >
                    <button
                      onClick={async (e) => {
                        e.stopPropagation();
                        try {
                          const res = await fetch(`/api/secure/deleteDesk?id=${desk.id}`, { method: "DELETE" });
                          if (!res.ok) {
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                ...
                <div
                  ...
                  onClick={async () => {
                    ...
                    try {
                      const res = await fetch(`/api/secure/getUnassignedItems?item=printer`);
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                ...
                <div
                  ...
                  onClick={async () => {
                    ...
                    setRoomItemLoading(true);
                    try {
                      const res = await fetch(`/api/secure/getUnassignedItems?item=almari`);
                      const items = await res.json();
                      setRoomUnassigned(items);
                      setRoomItemPickerOpen(true);
                    } finally {
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                ...
                <div
                  ...
                  onClick={async () => {
                    ...
                    setRoomItemLoading(true);
                    try {
                      const res = await fetch(`/api/secure/getUnassignedItems?item=bookshelf`);
                      const items = await res.json();
                      setRoomUnassigned(items);
                      setRoomItemPickerOpen(true);
                    } finally {
      ...
      {selectedRoomItem && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4"
          onClick={() => { setSelectedRoomItem(null); setRoomItemPickerOpen(false); setRoomUnassigned([]); setHeaderAssign(false); }}
        >
          <div
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-base font-semibold capitalize">{headerAssign ? `Select ${selectedRoomItem.type}` : `${selectedRoomItem.type} details`}</h3>
              <button
                onClick={() => { setSelectedRoomItem(null); setRoomItemPickerOpen(false); setRoomUnassigned([]); setHeaderAssign(false); }}
                className="p-1 rounded hover:bg-[color:var(--surface-muted)] cursor-pointer"
              >
            ...
            <div className="space-y-3">
              {!headerAssign && selectedRoomItem.item && (
                <>
                  <div className="text-sm">Name: {selectedRoomItem.item.name || `${selectedRoomItem.type}`}</div>
                  <div className="text-sm flex items-center gap-2">
                    <span className="text-gray-500">Status:</span>
                    <span
                      className={`inline-block rounded px-2 py-0.5 text-xs font-semibold text-white ${selectedRoomItem.item.status === "working" ? "bg-green-600" : "bg-red-500"}`}
                    >
                  ...
                  {selectedRoomItem.type === 'printer' && (
                    <div className="text-sm flex items-center gap-2">
                      ...
                      <span>{(selectedRoomItem.item as Printer).brand || '—'}</span>
                    </div>
                  )}
                  ...
                  <div className="flex gap-2 pt-2">
                    {selectedRoomItem.item.roomId ? (
                      <button
                        onClick={async () => {
                          setRoomItemLoading(true);
                          await fetch(`/api/secure/withdrawRoomItem?item=${selectedRoomItem.type}&id=${selectedRoomItem.item!.id}`);
                          setRoomItemLoading(false);
                      ...
                      <button
                        onClick={async () => {
                          try {
                            setRoomItemLoading(true);
                            const res = await fetch(`/api/secure/getUnassignedItems?item=${selectedRoomItem.type}`);
                            const items = await res.json();
                            setRoomUnassigned(items);
                            setRoomItemPickerOpen(true);
                          } finally {
              ...
              {roomItemPickerOpen && (
                <div className="mt-4 border-t border-[color:var(--border)] pt-3">
                  <div className="text-sm mb-2">Select {selectedRoomItem.type} to assign</div>
                  <div className="grid grid-cols-1 gap-2 max-h-64 overflow-auto">
                    {roomUnassigned.length > 0 ? (
                      roomUnassigned.map((u) => (
                        <button
                          ...
                          onClick={async () => {
                            ...
                            setRoomItemLoading(true);
                            await fetch(`/api/secure/assignRoomItem?item=${selectedRoomItem.type}&id=${u.id}&roomId=${roomId}`);
                            setRoomItemLoading(false);
                            setRoomItemPickerOpen(false);
                        ...
                        >
                          {assigningRoomItemId === u.id && roomItemLoading ? (
                            <div className="flex items-center gap-2"><LoadingSpinner /><span>Assigning...</span></div>
                          ) : (
                      ...
                      <div className="text-sm text-gray-500">No unassigned {selectedRoomItem.type}</div>
                    )}
                  </div>
                </div>
              )}

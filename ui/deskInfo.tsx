...
export default function DeskInfo({
  ...
  const [optimisticWithdraw, setOptimisticWithdraw] = useState<
    "monitor" | "cpu" | "ups" | ""
  >("");
  const [withdrawErrorItem, setWithdrawErrorItem] = useState<
    "" | "monitor" | "cpu" | "ups"
  >("");
  ...
  const handleItemWithdraw = async (itemId: number, item: string) => {
    try {
      ...
      if (item === "monitor" || item === "cpu" || item === "ups")
        setOptimisticWithdraw(item as "monitor" | "cpu" | "ups");
      ...
      setWithdrawErrorItem(
        item === "monitor" || item === "cpu" ? (item as "monitor" | "cpu") : ""
      );
                ...
                <div className="flex flex-col">
                  {unassignedItems.map((item) => (
                    <button
                      ...
                      disabled={
                        assigningId !== null || Boolean(withdrawLoading)
                      }
        ...
        {desk?.monitorId == null || optimisticWithdraw === "monitor" ? (
          ...
          <div className="flex flex-col mt-1 gap-y-2">
            <span className="px-2 py-0.5 text-sm font-medium leading-6 tracking-tight">
              Name: {desk.monitor?.name}
            </span>
            <span className="px-2 py-0.5 text-sm font-medium leading-6 tracking-tight">
              Brand: {desk.monitor?.brand}
            </span>
            <span className="px-2 py-0.5 text-sm font-medium leading-6 tracking-tight">
              Condition:
              <span
                className={`ml-2 inline-block rounded px-2 py-0.5 text-xs font-semibold text-white ${
                  desk.monitor?.status === "working"
                    ? "bg-green-500"
                    : "bg-red-500"
                }`}
              >
            ...
            <button
              onClick={() =>
                desk.monitorId && handleItemWithdraw(desk.monitorId, "monitor")
              }
              className="inline-flex items-center justify-center px-5 border-2 border-red-500 py-2.5 text-sm font-medium rounded-md text-[var(--btn-text)] bg-[var(--btn-bg)] transition-colors focus:outline-none focus:ring-2 focus:ring-red-400 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
              disabled={
                Boolean(withdrawLoading) || assigningId !== null || loading
              }
            >
              {withdrawLoading === "monitor" ? <LoadingSpinner /> : "Withdraw"}
            </button>
          </div>
        )}
      </div>
        ...
        {desk?.cpuId == null || optimisticWithdraw === "cpu" ? (
          <div className="text-center">
            <p className="text-secondary mb-4">
              This desk doesn't contain any CPU
            </p>
            {withdrawLoading === "cpu" ? (
              <div className="flex flex-col items-center gap-2 py-2">
                <LoadingSpinner />
          ...
          <div className="flex flex-col mt-1 gap-y-2">
            ...
            <span className="px-2 py-0.5 text-sm font-medium leading-6 tracking-tight">
              ...
              <span
                className={`ml-2 inline-block rounded px-2 py-0.5 text-xs font-semibold text-white ${
                  desk.cpu?.status === "working" ? "bg-green-500" : "bg-red-500"
                }`}
              >
                {desk.cpu?.status}
              </span>
        ...
        {desk?.upsId == null || optimisticWithdraw === "ups" ? (
          ...
          <div className="flex flex-col mt-1 gap-y-2">
            ...
            <span className="px-2 py-0.5 text-sm font-medium leading-6 tracking-tight">
              ...
              <span
                className={`ml-2 inline-block rounded px-2 py-0.5 text-xs font-semibold text-white ${
                  desk.ups?.status === "working" ? "bg-green-500" : "bg-red-500"
                }`}
              >
                {desk.ups?.status}
              </span>
            ...
            <button
              onClick={() =>
                desk.upsId && handleItemWithdraw(desk.upsId, "ups")
              }
              className="inline-flex items-center justify-center px-5 border-2 border-red-500 py-2.5 text-sm font-medium rounded-md text-[var(--btn-text)] bg-[var(--btn-bg)] transition-colors focus:outline-none focus:ring-2 focus:ring-red-400 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
              disabled={
                Boolean(withdrawLoading) || assigningId !== null || loading
            ...
            >
              {withdrawLoading === "ups" ? <LoadingSpinner /> : "Withdraw"}
            </button>
          </div>
        )}
      </div>
